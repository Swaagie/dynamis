...
Redis.readable('api', function api(dynamis) {
  ...
  dynamis.api('set', function set(key, value, ttl, done) {
    ...
    dynamis.execute(redis.database, redis.database.set, key, value, function next(error) {
      if (error || !ttl) return done.apply(this, arguments);

      dynamis.expire(key, ttl, done);
    });
  });
