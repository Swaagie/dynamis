...
Redis.readable('api', function api(dynamis) {
  ...
  /**
   * Authenticate against redis, since redis has no notion of users check how
   * arguments where supplied. This will keep the API smart and consistent.
   *
   * @param {String} username
   * @param {String} password
   * @param {Function} done completion callback
   */
  dynamis.api('auth', function auth(username, password, done) {
    if ('function' === typeof password) {
      done = password;
      password = username;
      username = null;
    }
